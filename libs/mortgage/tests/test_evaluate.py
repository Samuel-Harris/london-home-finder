...
def test_four_hundred_thousand_forward(sample_buyer: Buyer) -> None:
    point = evaluate_at_price(AUGUST_2026, sample_buyer, 400_000)

    assert point.lisa_applied == 10_000
    assert point.stamp_duty == 5_000
    assert point.fees == 2_500
    assert point.usable_deposit == 42_500
    assert point.loan == 357_500
    assert point.ltv_band == Decimal("0.90")
    assert point.rate == Decimal("0.052")
    assert point.monthly is not None
    assert abs(point.monthly - 2132) <= 20
    assert point.constraints.viable
    assert point.constraints.binding is None


def test_five_hundred_thousand_applies_lisa_penalty(sample_buyer: Buyer) -> None:
    point = evaluate_at_price(AUGUST_2026, sample_buyer, 500_000)

    assert point.lisa_applied == 7_500
    assert point.stamp_duty == 10_000
    assert point.usable_deposit == 35_000
    assert point.loan == 465_000
    assert point.constraints.viable is False
    assert point.constraints.binding == "income"


def test_scan_maps_evaluate_at_price(sample_buyer: Buyer) -> None:
    bounds = PriceBounds(min_price=400_000, max_price=420_000)
    rows = scan(sample_buyer, AUGUST_2026, bounds, 10_000)

    assert tuple(row.price for row in rows) == (400_000, 410_000, 420_000)
    for row in rows:
        assert row == evaluate_at_price(AUGUST_2026, sample_buyer, row.price)


def test_scan_includes_unaligned_max_price(sample_buyer: Buyer) -> None:
