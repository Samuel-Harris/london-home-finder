...
from lhf.scraper.onthemarket.shards import ShardFilter, houses_spine, window_property_kinds
...
def checkpoint_path(database_path: str | Path) -> Path:
    return Path(f"{database_path}.onthemarket-scrape-checkpoint.json")


def new_checkpoint(window: IngestWindow, max_pages: int | None) -> Checkpoint:
    ...
    return Checkpoint(
        ...
        phase="search",
        pages_used=0,
        pending=list(houses_spine(window.min_bedrooms)),
        active=None,
        cards=[],
        details={},
    )
...
def load_checkpoint(path: Path) -> Checkpoint:
    if not path.is_file():
        raise ValueError(f"no onthemarket scrape checkpoint to resume at {path}")
    try:
        payload: object = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise ValueError("onthemarket scrape checkpoint is invalid") from exc
    try:
    ...
    if version != CHECKPOINT_VERSION:
        raise ValueError(f"onthemarket scrape checkpoint version {version} is not supported")
    try:
        return _from_asdict(Checkpoint, data)
