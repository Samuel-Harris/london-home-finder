...
from lhf.scraper.zoopla.shards import SearchFilter, search_path
...
def checkpoint_path(database_path: str | Path) -> Path:
    return Path(f"{database_path}.zoopla-scrape-checkpoint.json")
...
def new_checkpoint(window: SearchFilter, max_pages: int | None) -> Checkpoint:
    return Checkpoint(
        ...
        window=window,
        max_pages=max_pages,
        search_url_base=search_path(window),
        phase="search",
        pages_used=0,
...
def load_checkpoint(path: Path) -> Checkpoint:
    ...
    try:
        ...
        raise ValueError("scrape checkpoint is invalid") from exc
    if state.search_url_base != search_path(state.window):
