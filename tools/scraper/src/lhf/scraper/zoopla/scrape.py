...
from lhf.scraper.zoopla.checkpoint import (
    ActiveShard,
    Checkpoint,
    checkpoint_path,
    clear_checkpoint,
...
from lhf.scraper.zoopla.detail import PropertyDetail, parse_property_data
from lhf.scraper.zoopla.http import ZOOPLA_ORIGIN, Fetcher, FetchError
from lhf.scraper.zoopla.map_listing import map_listing
from lhf.scraper.zoopla.search import SearchCard, SearchPage, parse_search_page
from lhf.scraper.zoopla.shards import SearchFilter, filter_label, search_url, split_filter

MAX_SEARCH_PAGE = 40
PAGE_RESULT_CAP = 1000
...
def scrape(
    ...
    *,
    ...
    count = ListingRepository(create_session_factory(database_path)).replace_zoopla(drafts)
...
def _walk_search(fetcher: Fetcher, state: Checkpoint, path: Path) -> None:
    ...
    while True:
        ...
        page = _search_page(fetcher, state.active.filter, state.active.next_page)
        if page is None or not page.properties:
            state.active = None
            save_checkpoint(path, state)
            continue
        if state.active.next_page == 1:
            if _shard_overflows(page):
                try:
        ...
        state.pages_used += 1
        last_page = page.page_number_max or MAX_SEARCH_PAGE
        if last_page > MAX_SEARCH_PAGE:
            last_page = MAX_SEARCH_PAGE
        state.active.next_page += 1
        if state.active.next_page > last_page:
            state.active = None
        save_checkpoint(path, state)
    state.phase = "details"
...
def _shard_overflows(page: SearchPage) -> bool:
    if page.result_count is None:
        return (page.page_number_max or 0) >= MAX_SEARCH_PAGE and len(page.properties) > 0
    return page.result_count > PAGE_RESULT_CAP


def _search_page(
    fetcher: Fetcher, search_filter: SearchFilter, page_number: int
) -> SearchPage | None:
    url = search_url(search_filter, page_number)
    try:
        html = fetcher.get(url)
    except FetchError as exc:
        if page_number > 1 and exc.status == 404:
            return None
        raise
    try:
...
def _fetch_detail(fetcher: Fetcher, card: SearchCard) -> PropertyDetail | None:
    url = f"{ZOOPLA_ORIGIN}/for-sale/details/{card.listing_id}/"
    try:
